...
const teamname = "KingNU/ICPC_notebook";   // チーム名 + 大学名 (各ページ左上に表示)
const fontsize = "7.4pt";       // フォントサイズ
const orientation = "portrait";     // portrait (縦長) / landscape (横長)
const num_columns = 1;          // 何段組みか
...
function addCode(folder, filename) {
    ...
    // 行番号を付ける（.md は除外）
    let highlight_with_line_numbers = highlight_text;
    if (!filename.endsWith(".md")) {
        const lines = highlight_text.split('\n');
        highlight_with_line_numbers = lines
            .map((line, i) =>
                `<span class="line"><span class="num">${i + 1}</span>${line}</span>`
            )
            .join('\n');
    }
    ...
    if (filename.endsWith(".md")) {
    // Markdown はページを完全に独立させる
    codeContent += `
    <div class="markdown-page">
        ${marked.marked(text, { gfm: true, breaks: true })}
    </div>`;
    codeContent += `<div class="page-break"></div>`;

    return;
    } else {
        codeContent += `
    <pre><code class="line-numbers">${highlight_with_line_numbers}</code></pre>
`;
    }
...
/* 行番号の余白を広げる */
pre code.line-numbers .num {
    display: inline-block;
    width: 3.5em;   /* ← ここを広げればもっと間が空く */
    padding-right: 2.0em; /* ← 数字とコードの間に余白 */
    user-select: none;
    color: #888;
    text-align: right;
    margin-right: 0; /* margin-right は不要 */
}
